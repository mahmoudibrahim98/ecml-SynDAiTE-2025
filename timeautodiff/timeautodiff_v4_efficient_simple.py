...
#         if t is None or time_info is None:
...
def consistency_regularization(x, perturbed_x, model, is_autoencoder=True, t=None, cond=None, time_info=None, i=None):
    """
    Encourage model output consistency between original and perturbed inputs.
    """
    if is_autoencoder:
        original_output = model(x)[0]
        perturbed_output = model(perturbed_x)[0]
        consistency_loss = 0
        num_types = 0
            ...
            if key in perturbed_output:
                ...
                if key == 'bins':
                    consistency_loss += F.mse_loss(
                        torch.sigmoid(original_output[key]), 
                        torch.sigmoid(perturbed_output[key])
                    )
                elif key == 'cats':
                    # Fix: Use KL divergence between probability distributions
                    for orig_cat, pert_cat in zip(original_output[key], perturbed_output[key]):
                        orig_probs = F.softmax(orig_cat, dim=-1)
                        pert_probs = F.softmax(pert_cat, dim=-1)
                        consistency_loss += F.kl_div(
                            F.log_softmax(pert_cat, dim=-1), 
                            orig_probs, 
                            reduction='batchmean'
                        )
                elif key == 'nums':
        ...
        # Normalize by number of output types
        if num_types > 0:
            consistency_loss = consistency_loss / num_types
                    
    else:
        # Fix: Pass correct parameters to diffusion model
        if t is None or time_info is None or i is None:
            raise ValueError("For BiRNN_score model, t, i, and time_info must be provided.")

        original_output = model(x, t, i, cond, time_info)
        perturbed_output = model(perturbed_x, t, i, cond, time_info)
        consistency_loss = F.mse_loss(original_output, perturbed_output)
...
def train_autoencoder(processed_data, channels, hidden_size, num_layers, lr, weight_decay, 
    ...
    def process_batch(batch_data):
        ...
        if consistency_weight > 0:
            perturbed_inputs = batch_data + 0.01 * torch.randn_like(batch_data).to(device)
...
def train_diffusion(latent_features, cond_tensor, time_info, hidden_dim, num_layers, 
    ...
    with Progress() as progress:
        ...
        for epoch in range(n_epochs):
            ...
            # Consistency Regularization
            if consistency_weight > 0:
                perturbed_x = x[batch_indices, :, :] + 0.01 * torch.randn_like(x[batch_indices, :, :]).to(device)
                cond_batch = cond_tensor[batch_indices,:,:] if cond_tensor is not None else None
                i_batch = torch.randint(0, diffusion_steps, size=(x[batch_indices].shape[0],))
                i_batch = i_batch.view(-1, 1, 1).expand_as(x[batch_indices][...,:1]).to(x.device)
                consistency_loss = consistency_regularization(x[batch_indices, :, :], perturbed_x, model,False, t, cond_batch, time_info[batch_indices,:,:],i_batch)
            else:
                consistency_loss = 0
